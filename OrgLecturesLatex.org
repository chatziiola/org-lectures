...
To ensure proper functionality and configuration, when the minor mode starts:
- The file is exported to LaTeX using the default `ox-latex` backend.
- A buffer-local hook is added to keep the `tex` file updated, achieved by `org-lectures-export-to-latex` (a simple wrapper to the backend) running on save.
- A `latexmk` process starts, continuously publishing the `tex` file to `pdf`. During its first execution (after the `pdf` is first created), the default `pdf viewer` will also open.
