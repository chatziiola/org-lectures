...
The index cache needs to be invalidated whenever a course or lecture file is
added, modified, or removed. The choice of strategy for detecting these changes
involves a trade-off between robustness and performance.

While a hash-based approach (calculating and comparing hashes of all file
contents), as used by systems like =ox-publish=, offers maximum robustness, it
comes with a significant performance cost. It would require reading every single
lecture file on every interactive command invocation, leading to noticeable
delay.

Instead, this module uses a modification-time based approach. It recursively
scans all =.org= files in the lecture directory and compares their modification
times against the index file's modification time. If any file is newer, the
index is rebuilt. This is much faster as it only requires file metadata from the
operating system, not the file contents.

This strategy reliably detects changes in any file (including those in
subdirectories) and is fast enough for the interactive nature of
=org-lectures=.
